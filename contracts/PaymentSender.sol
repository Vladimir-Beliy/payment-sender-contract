...
contract PaymentSender is Ownable, Pausable, EIP712 {
    ...
    constructor(address voucherSigner_, address paymentToken_) EIP712("Payment Sender", "1") {
    ...
    function voucherSigner(address voucherSigner_) public onlyOwner {
        require(voucherSigner_ != address(0), "PaymentSender: voucher signer is zero address");
        require(voucherSigner_.code.length == 0, "PaymentSender: voucher signer is contract address");

        _voucherSigner = voucherSigner_;
    }

    function paymentToken(address paymentToken_) public onlyOwner {
        require(paymentToken_ != address(0), "PaymentSender: payment token is zero address");
        require(paymentToken_.code.length > 0, "PaymentSender: payment token isn't contract address");
    ...
    function _verifyVoucher(uint256 nonce_, uint256 amount, bytes calldata voucher) private {
